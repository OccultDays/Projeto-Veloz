...
class IngredienteViewSet(viewsets.ModelViewSet):
    """
    CRUD completo para os ingredientes do controle de estoque.
    """
    ...
    def atualizar_estoques(self, request):
        return self._executar_atualizacao_em_lote(request)

    @action(detail=False, methods=["post"], url_path="atualizar-em-lote")
    def atualizar_em_lote(self, request):
        return self._executar_atualizacao_em_lote(request)

    def _executar_atualizacao_em_lote(self, request):
        """
        Atualiza o estoque atual de múltiplos ingredientes em lote.
        Aceita tanto {"itens": [{"id": 1, "estoque_atual": 15.0}, ...]} quanto [{"id": 1, "estoque_atual": 15.0}, ...].
        """
        dados = request.data
...
def exportar_lista_compras_txt(request):
    ...
    response["Content-Disposition"] = 'attachment; filename="lista_compras.txt"'
    return response
...
def index_view(request):
    ...
    return HttpResponse(
        "<h1>Controle de Estoque e Reposição - Backend Django Ativo</h1>"
