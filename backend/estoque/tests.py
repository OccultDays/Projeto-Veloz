...
class CalculoReposicaoServiceTestCase(TestCase):
    ...
    def test_falta_no_meio_do_mes_com_margem_20_porcento(self):
        """
...
class DjangoIntegrationTestCase(TestCase):
    ...
    def test_atualizar_estoque_em_lote(self):
        """
        Verifica a atualização simultânea do estoque de múltiplos ingredientes em lote.
        """
        payload = {
            "itens": [
                {"id": self.farinha.id, "estoque_atual": 45.5},
                {"id": self.acucar.id, "estoque_atual": 18.0},
            ]
        }
        resposta = self.client.post("/api/ingredientes/atualizar-em-lote/", payload, format="json")
        self.assertEqual(resposta.status_code, 200)
        self.assertEqual(resposta.data["total_atualizados"], 2)

        self.farinha.refresh_from_db()
        self.assertEqual(self.farinha.estoque_atual, Decimal("45.50"))
        self.acucar.refresh_from_db()
        self.assertEqual(self.acucar.estoque_atual, Decimal("18.00"))

        # Testa também o alias /api/ingredientes/atualizar-estoques/
        payload_lista = [
            {"id": self.farinha.id, "estoque_atual": 40.0}
        ]
        resposta_alias = self.client.post("/api/ingredientes/atualizar-estoques/", payload_lista, format="json")
        self.assertEqual(resposta_alias.status_code, 200)
        self.assertEqual(resposta_alias.data["total_atualizados"], 1)

        self.farinha.refresh_from_db()
        self.assertEqual(self.farinha.estoque_atual, Decimal("40.00"))
